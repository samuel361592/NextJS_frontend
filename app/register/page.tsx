...
export default function RegisterPage() {
    ...
    const evaluatePasswordStrength = (pwd: string): string => {
        ...
        if (/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(pwd)) return '強';
        return '弱';
    };
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100 px-4 justify-center items-center">
            {/* NavBar */}
            <nav style={{ backgroundColor: '#4C8BF5', padding: '10px', position: 'fixed', top: 0, width: '100%', zIndex: 10 }}>
            ...
            <div className="w-full max-w-md p-8 bg-white rounded shadow-md">
                <h1 className="text-2xl font-bold text-center text-gray-800 mb-6">註冊帳號</h1>
                ...
                <div className="mb-6">
                    ...
                    {password.length > 0 && password.length < 8 && (
                        <p className="text-red-600 text-sm mt-1">密碼太短，至少需要 8 個字元</p>
                    )}

                    {/* 密碼強度條顯示 */}
                    {password && (
                        <div className="mt-2">
                            <p className="text-xs text-gray-600">密碼強度：{passwordStrength}</p>
                            <div className="w-full h-2 bg-gray-200 rounded mt-1">
                                <div
                                    className={`
                                        h-full rounded transition-all duration-300
                                        ${passwordStrength === '強' ? 'w-full bg-green-500' :
                                            passwordStrength === '中' ? 'w-2/3 bg-orange-400' :
                                                'w-1/3 bg-red-500'}
                                            `}
                                />
                            </div>
                ...
                <button
                    ...
                    disabled={!isValidEmail(email) || password.length < 8 || password.length > 60}
